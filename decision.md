...
**因此 W1b 不推翻，素材管理入口不开。** 连带保住三件事：① 后端红线 5 不动（版式库仍在 repo，不必发明一类「必须存在于生产的产品参考资料」）；② 300dpi / 透明通道的品控门槛不转嫁给园方；③ W13「版面品质由我们负责」原样成立。
...
**对 backend 的影响**：`IF 请求发布 AND 任一 db_school_book_section 的声明槽位未传满或声明的 body_text 为空, return 422` 并回传 `section_id` 与缺失项。

### 9. 预览版心几何修正 —— 与后端 R26 是同一个错
...
### 10. `spec-handoff.md` 移出 `Archive/` 并补齐注册表
