...
## 2026-08-13：F19 年级学期模板槽位覆盖

本节以 `hualong-backend/DECISIONS.md` F19 为权威，覆盖本文此前「禁止年级模板」与全园单一 `book_template_code` 的口径。

- 版式目录固定为小／中／大班 × 春／秋 × A／B 共 12 套 released pack。管理端顶部用春／秋两个 tab 切换；每个 tab 下纵向排列小／中／大班三张卡片，每张右侧同时展示 A／B 两套模板的预览图、说明、选择按钮与正本预览。
- 槽位不按学年复制。新升中班继续读取当前「中班 × 对应季节」槽，因此自然沿用上一届中班所用配置，除非管理员主动改选。
- ~~改选 assignment 与园所固定内容的一次性 d2 发布相互独立；管理员可在之后单槽改选~~ —— **已由 2026-08-14 的 F20 取消**，见下节。既有册只保存建册时的 `pack_code`，不得被回写这一条不变。
- 12 套 pack 的主题文字由年级发展阶段与春秋季决定，A／B 主要提供配色和美术差异；册末寄语改为全园同一学期共用的 `db_school_term.growth_book_message`，由管理端维护，不再读取逐幼儿 `db_teacher_message`。
- 学期寄语最多 500 字；本学期任一班级首次锁定 compilation 后永久唯读，避免已经定稿的正本内容漂移。
- 园长寄语不再是固定内容，也没有专属字段、年级变体或模板槽位开关。新增园所栏目与教师新增班级栏目只在名称输入处提示“园长寄语”作为可选例子，是否创建由用户明确决定。

## 2026-08-14：F20 园所设置改为按学期锁定，发布即版本

本节以 `hualong-backend/DECISIONS.md` F20 为权威，覆盖 F16 的「首次 d2 后永久唯读」与上一节的 assignment 独立改选。

- **起因**：F16 与 F19 各自成立，叠起来的后果是幼儿园终身只发布一次成长册设置 —— 不能换封面、不能改园所介绍、不能新增园所栏目，六个 pack 槽位选完定死，12 套 released pack 有 6 套永远用不到。这不是任何单条决定的本意。
- **锁定周期改为学期。** `db_school.book_setting_status` 仍是 d1／d2 两态且不带 term 维度；新增 `d2 → d1` 撤回，前置条件是当前园所学期没有任何 `db_growth_book_compilation` 处于 `e2`。有任一 e2 即 409，按钮保留禁用并解释「本学期已有班级锁定编册，下学期才能改」。**没有自动解锁、没有定时任务** —— 管理员不动就自然沿用上学期内容，这是最省管理层动作的形态。
- **六槽 assignment 回到同一个发布包**，共用同一个 d1／d2，不另立状态机、不做发布后单槽 CAS 改选。两条独立状态机会产生「内容已冻结但槽位还能改」的中间态，而槽位直接决定正本每一页的版式，那个中间态没有可解释的语意。
- **发布即版本。** 每次成功 `d1→d2` 插入一列不可变 `db_school_book_release`，冻结封面／Logo／园所介绍／版式 code／六槽 assignment；园所栏目由 `db_school_book_section.release_id` 逐行归属；`db_growth_book.book_release_id` 建册时绑定且永不回写。composer 一律从 release 读，不读 live 工作区。这是「旧册保留定稿当时那一版」的唯一结构解，也推翻了 Q62-j43／j53 的「共享来源读最新」—— 那条成立的前提是园所内容永不变，前提已经没了。目标表数 59 → 60。
- `book_setting_published_at` 语意由「唯一发布时间」改为「最近一次发布时间」，UI 称「最近发布」；逐次历史读 `db_school_book_release.published_at`。
- **学期寄语不进发布包**，也不进 release 快照 —— `db_school_term` 逐学期一行，term 行本身就是它自己的历史。因此它与 `book_setting_status` 无关，d2 期间仍可单独保存，唯一约束是本学期尚无 e2。**管理端必须给它独立的保存按钮**：此前它与「保存草稿／发布设置」共用，而 d2 后保存草稿一律 409，等于第二学期的寄语完全没有写入路径。
- 「本学期有无 e2」成为一个 predicate、三处复用（能否撤回、能否改园所内容、能否改本学期寄语），服务端只实作一次。它依赖 `db_school_term` 派生当前学期，**无进行中学期时三者一律 409** —— 园曆因此从成长册的软依赖变成硬依赖。
