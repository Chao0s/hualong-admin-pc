...
prototype_content = HTML 中模板卡片缩略图、示例园所介绍、示例园所栏目「园所大事记」、logo 与封面照片均为 Mock
static_ui_content = 模块名、字段标签、年级页签名、栏目版式名、上传规格说明、按钮名和空状态文案
business_seed = NONE
production_initial_business_objects = db_school 本节内容字段 NULL；intro_layout_code 取版式库默认值 photo；book_setting_status=d1；db_school_term.growth_book_message NULL；db_school_book_section EMPTY
...
| 10 | 选择年级学期模板 | Select Grade-season Template | btn_admin_gb_select_template | db_school_book_template_assignment | grade, term_season, pack_code | update target assignment with CAS |
...
| 13 | 春季 | Spring Tab | btn_admin_gb_spring | db_school_book_template_assignment | NULL | local tab; show k1/k2/k3 |
| 14 | 秋季 | Autumn Tab | btn_admin_gb_autumn | db_school_book_template_assignment | NULL | local tab; show k1/k2/k3 |
| 15 | 预览正本 | Preview Full Book | btn_admin_gb_pack_preview | layout pack | pack_code | open exact pack preview; dynamic per candidate |
| 16 | 保存草稿 | Save Draft | btn_admin_gb_save_draft | db_school | validated form + file_id | 仅 d1 可 persist；d2 后隐藏 |
| 17 | 发布 | Publish Setting | btn_admin_gb_publish | db_school + db_school_book_section + db_file_ref | validated complete candidate | 唯一一次单事务发布为 d2；之后只读 |
| 18 | 园所介绍 · 确认 | Commit Intro | btn_admin_gb_commit_intro | db_school | intro textarea | 仅 d1 写草稿；d2 后只读 |
| 19 | ＋ 新增栏目 | Add School Section | btn_admin_gb_sec_add | db_school_book_section | NULL | open section modal; 园长寄语仅为名称提示候选 |
| 20 | 栏目照片 · 上传 | Upload Section Photo | btn_admin_gb_sec_upload | db_file | local file, slot index | upload and return file_id |
| 21 | 栏目 · 保存 | Save Section | btn_admin_gb_sec_save | db_school_book_section | validated form | create/update |
| 22 | 栏目 · 删除 | Delete Section | btn_admin_gb_sec_delete | db_school_book_section | section id | delete and re-anchor |
| 23 | 栏目 · 取消 | Cancel Section | btn_admin_gb_sec_cancel | db_school_book_section | NULL | close modal |
| 24 | 园所介绍照片 · 上传 | Upload Intro Photo | btn_admin_gb_intro_upload | db_file | local file, slot index | upload and return file_id |
| 25 | 学期寄语 · 确认 | Commit Term Message | btn_admin_gb_term_message | db_school_term | current school term + message | 首个 compilation 锁定前保存，之后唯读 |
...
|---|---|---:|
| 年级季节模板 | db_school_book_template_assignment.pack_code | 六个槽位各 1 |
...
| 园所介绍 · 照片 | db_file_ref(owner_object='db_school', usage_key='book_intro') | 草稿 0:k；发布须传满 intro_layout_code 声明的槽位数 |
| 学期寄语 | db_school_term.growth_book_message | 1；trim 后 1—500 字，本学期首个 compilation 锁定前可写 |
...
upload_constraint = logo、封面照片、园所介绍照片、栏目照片一律走 db_file 通用上传；本 spec 不新增文件表，也不为美术素材开上传口
...
| 栏目版式卡片 | admin_gb_section_layout_card | layout library manifest | NULL | 1:k | select layout_code（园所栏目出四种，园所介绍只出带正文的三种） |
...
| 预览页 | admin_gb_preview_page | db_school + db_school_term + db_school_book_section + layout library | current draft values | 0:k | render only |
...
object_status (对象状态) = REUSE + EXTEND
...
book_template_code_removed (F19) = 不再存 db_school；每册 pack_code 从六槽 assignment 在建册时解析
...
file_id (园所介绍照片ID), 0:k, integer, ui=growth_book_setting.intro_photo_upload
...
园所填的是内容占位符：logo、整体照片、园所名称、园所介绍，以及自行新增的园所栏目
...
assignment.pack_code 的合法取值来自后端仓库版式库的 released manifest（db/layout/*.json），随部署走、跟 git 走 code review
管理端六个常设槽只存所选代码，不存版面与美术素材；理由见 layout-spec 7.3.5 与红线 5
released 目录必须恰有 3 年级 × 春秋 × A/B 共 12 套；缺套时对应槽不可保存

cover_layout_id_note (F19 覆写):
封面与内页继续属于同一个 pack，但 pack_code 存在 grade × term_season assignment，并在建册时复制到 db_growth_book；db_school 不再保存全园单一模板代码
book_cover 只保存园所上传照片与标题占位内容，不承担版式选择

preset_section_layout_rule (园所介绍可选版式 / 2026-08-03 前端评审):
园所介绍与园所栏目使用同一套版式；介绍只出带正文的三种（纯文字／上图下文／两图并排），默认 photo
照片槽位数由 layout_code 声明，**不是可写字段、不落列**，同 school_section_layout_rule
填写区旁、正本预览与教师端渲染走同一个渲染器
...
reuse_source (复用来源) = hualong-backend DECISIONS.md F3／F4；本 spec 只扩充成长册字段
school_term_id (园所学期ID), 1:1, integer, ui=context.hidden
school_id (园所ID), 1:1, integer, ui=context.hidden
growth_book_message (学期寄语), 1:1, max_len=500, ui=growth_book_setting.term_message

term_source_rule = 页面只编辑服务端按园所时区派生的当前园所学期；无当前学期时唯读显示“当前不在学期内”，不回退最近结束学期写入
...
assignment_id (模板选择ID), 1:1, integer, ui=growth_book_setting.assignment.hidden
school_id (园所ID), 1:1, integer, ui=context.hidden
grade (年级), 1:1, k1|k2|k3, ui=growth_book_setting.assignment.grade
term_season (学期季节), 1:1, spring|autumn, ui=growth_book_setting.assignment.season
pack_code (模板代码), 1:1, released pack, ui=growth_book_setting.assignment.template
revision (并发修订号), 1:1, bigint, ui=context.hidden
updated_by_admin_id (更新管理员), 1:1, integer, ui=context.hidden
updated_at (更新时间), 1:1, datetime, ui=growth_book_setting.assignment.updated_at
...
content_safety_rule (F16) = 唯一一次发布前，管理员必须完整预览园所介绍、封面、Logo、园所栏目文字与全部图片，明确点击发布即完成当次人工把关。不调微信 API，不建另一 admin 复核、待审状态、queue 或 review action
full_preview_rule (Q62-j69) = 每个模板候选提供对应 released pack 的正本预览；园所内容唯一发布前仍须逐页预览当前完整 candidate。只校验本次明确确认与当前 candidate 一致，不建审核证据、操作或版本历史
...
preset_section_completeness_rule (园所介绍的齐备判定):
园所介绍与园所栏目使用同一套判定，作用于发布这一步，不挡保存草稿；介绍正文发布前必填，版式声明了照片槽位则须传满，纯文字版式不要求照片
...
db_growth_book_compilation|db_growth_book = REUSE Teacher App 05 home-school-spec.md；管理端只读、不写这两张表
db_school_term = REUSE 后端 F3／F4 园所学期日历；本页只在当前学期首个 compilation 锁定前写 growth_book_message
...
db_school book_template_code —— F19 作废，不落列；改由 db_school_book_template_assignment.pack_code 承载六个槽位
...
db_file_ref ADD owner_object='db_school' + usage_key='book_intro' —— 复用既有多态文件关联承载园所介绍的 0:k 照片，不向 db_school 增数组列
db_school_term ADD growth_book_message(1:1, max_len=500) —— F19 全园学期寄语；首个 compilation 锁定后唯读
db_school_column_growth_note (登记不改判) = 园长寄语专属三列已经取消，不进入待迁移 DDL；其余成长册字段按本节最终状态落地
preset_section_file_note (0:k 的落列) = 园所介绍与 db_school_book_section 的 0:k 照片统一走 db_file_ref；只存文件引用，不向业务表复制数组列
extension_rule = extend the canonical db_school only; app-prefixed school/setting copies are FORBIDDEN
preset_section_rule = 不新增可开关的园长寄语预设栏目；该名称只作为 db_school_book_section 新增栏目的界面提示，不自动建立业务对象


[EMPTY_STATE]

IF 任一 grade × term_season assignment 未选, 对应槽预览返回 [] AND empty_title=请先选择 A 或 B 模板
...
IF assignment.pack_code NOT_IN released layout manifest 或其 grade／term_season 与目标槽不符, return 400
...
IF candidate hash 未完成当前完整园所内容的逐页预览, return 422
IF 删除园所栏目 AND book_setting_status!=d1, return 409
IF layout_code NOT_IN layout manifest 的栏目版式清单, return 400
IF intro_layout_code NOT_IN layout manifest 中声明 body=1 的版式, return 400
